...
var API_BASE_URL = process.env.OPI_API_URL || process.env.AGENTBOX_API_URL || "https://0pi.dev";
var LOG_DIR = process.env.OPI_LOG_DIR || process.env.AGENTBOX_LOG_DIR || (__dirname ? join(__dirname, "logs") : "./logs");
var LOG_FILE = LOG_DIR ? join(LOG_DIR, "mcp-conversations.jsonl") : null;
...
async function createSharedWorkspace(agent_id, data, intent = null, ttl_seconds = 7200) {
  try {
    ...
    const response = await axios_default.post(`${API_BASE_URL}/dump`, payload, {
      headers: {
...
async function getWorkspace(workspace_id) {
  try {
    const response = await axios_default.get(`${API_BASE_URL}/w/${workspace_id}`, {
      timeout: 5e3
    });
